...
func newDeleteCommand(cli cli.CLI) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Short:         "Delete http route rewrite configuration of a service",
...
func (c *deleteCommand) run(cli cli.CLI, options *deleteOptions) error {
	...
	if matchedRoute.Rewrite == nil {
		log.Infof("rewrite is not set for %s", options.serviceName)
		return nil
	}
	...
	if cli.Interactive() {
		...
		confirmed := false
		err = survey.AskOne(&survey.Confirm{Message: "Do you want to DELETE the rewrite configuration?"}, &confirmed)
		if err != nil {
	...
	if !r2 {
		return errors.New("unknown error: cannot delete rewrite configuration")
	}

	log.Infof("rewrite configuration set to %s successfully deleted", options.serviceName)
