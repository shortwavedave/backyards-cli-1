...
func NewRequireCommand(cli cli.CLI) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:           "require [[--resource=]mesh|namespace|namespace/servicename[:[portname|portnumber]]]",
		Short:         "Set mTLS policy setting for a resource to STRICT",
...
func (c *requireCommand) run(cli cli.CLI, options *mTLSOptions) error {
	var err error
	...
	return setMTLS(cli, options, client, ModeStrict)
}
