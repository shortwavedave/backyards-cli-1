...
func NewUnsetCommand(cli cli.CLI) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:           "unset [[--resource=]namespace|namespace/servicename[:[portname|portnumber]]]",
		Short:         "Delete mTLS policy setting for a resource",
...
func (c *unsetCommand) run(cli cli.CLI, options *mTLSOptions) error {
	var err error
	...
	return unsetMTLS(cli, options, client)
}
