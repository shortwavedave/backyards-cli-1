...
func NewAllowCommand(cli cli.CLI) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:           "allow [[--resource=]mesh|namespace|namespace/servicename[:[portname|portnumber]]]",
		Short:         "Set mTLS policy setting for a resource to PERMISSIVE",
...
func (c *allowCommand) run(cli cli.CLI, options *mTLSOptions) error {
	var err error
	...
	return setMTLS(cli, options, client, ModePermissive)
}
