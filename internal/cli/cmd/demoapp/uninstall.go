...
It can only dump the removable resources with the '--dump-resources' option.`,
  ...
  backyards canary uninstall install -n custom-istio-ns`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			return cli.IfConfirmed("Uninstall demo application. This command will destroy resources and cannot be undone. Are you sure to proceed?", func() error {
				return c.run(cli, options)
			})
		},
	}
...
func (c *uninstallCommand) run(cli cli.CLI, options *UninstallOptions) error {
	objects, err := getBackyardsDemoObjects(options.namespace, false)
	if err != nil {
		return err
	}
